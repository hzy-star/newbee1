    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="500px" :before-close="handleClose">
      <el-form ref="formRef" :model="formData" :rules="formRules" label-width="100px">
        <el-form-item label="阈值名称" prop="name">
          <el-input v-model="formData.name" placeholder="请输入阈值名称" :disabled="isView" />
        </el-form-item>
...
// 响应式数据
const strategyList = ref<StrategyThreshold[]>([])
const strategyListBackUp = ref<StrategyThreshold[]>([])
...
const formRules: FormRules = {
  name: [
    { required: true, message: '请输入阈值名称', trigger: 'blur' }
  ],
  ruleFile: [
...
// 编辑阈值
const handleEdit = (row: StrategyThreshold) => {
...
// 查看阈值
const handleView = (row: StrategyThreshold) => {
  formData.value = { ...row } // 包含id
  dialogTitle.value = '查看阈值'
  isView.value = true
  dialogVisible.value = true
}

// 删除阈值
const handleDelete = async (row: StrategyThreshold) => {
  ElMessageBox.confirm('确定要删除这个阈值吗？', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
    try {
      const data = await reqDeleteStrategyThreshold({ id: row.id })
