<template>
    <div class="csv-box">
        ...
        <div class="top-line">
            ...
            <input v-model="path" type="text" placeholder="请输入 CSV 地址" class="top-line__input top-line__input--path" />
        ...
        <div v-if="batchDialogVisible" class="dialog-mask">
            <div class="dialog dialog--batch">
                <div class="dialog__header">
                    {{ batchDialogType === 'replace' ? '替换所有匹配行' : '新增行' }}
                </div>
                <div class="dialog__body dialog__body--scroll">
                    <div class="batch-tip">
                        按当前搜索条件
                ...
                <div class="dialog__footer">
                    <button class="btn btn--plain" @click="closeBatchDialog">
                        取消
                    </button>
                    <button class="btn btn--primary" @click="submitBatchDialog">
...
// 如果你已经全局注册了 vxe-table，可以不在这里单独 import。
// 这里仅作为类型/按需引入参考。
...
// vxe-table 需要对象数组，这里做一次映射
// 注意：这里会有 10 万行对象，如果性能仍有问题，可以再换成“dummy 索引 + 单元格用 $rowIndex 访问 tableData”的写法
...
const loadHeader = async () => {
    ...
    try {
        const params: any = { path: path.value }
...
const queryList = async () => {
    ...
    try {
        const payload: any = { path: path.value }
...
const deleteSingleRow = async (rowIndex: number) => {
    // if (!window.confirm('确认删除该行数据吗？')) return
    ElMessageBox.confirm('确认删除该行数据吗？', '提示', {
        ...
        const payload: any = { path: path.value, filters }
        try {
            const res = await reqCsvEditDeleteUrl(payload)
            if (res?.code !== 200) {
                ElMessage.error(res?.errMsg || '删除失败，请稍后重试')
...
const submitBatchDialog = async () => {
    ...
    const base: any = { path: path.value }
    ...
    try {
        if (batchDialogType.value === 'add') {
            ...
            //   path: 'oss://...xxx.csv',
            //   newRows: [
            //     ["MY","all","chartboost","blacklist","adx","1","ios","abc"],
            //     ["US","all","chartboost","grtr","adx","2","ios","szxc"],
            //   ]
            // }
            ...
            //   filters: { country: ["MY"] },
            //   newRows: [
            //     ["CB","all","chartboost","blacklist","adx","1","ios","1"],
            //     ["CB","all","chartboost","blacklist","adx","2","ios",""]
            //   ]
            // }
            const res = await reqCsvEditReplaceUrl(payload)
