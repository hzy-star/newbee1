<template>
    <div class="csv-box">
        ...
        <!-- 只覆盖 csv-box 的蒙层 -->
        <div v-if="boxDisabled" class="csv-box-mask">
            <div class="csv-box-mask__content">
                <div class="csv-box-mask__text">页面正在开发中，暂不可用！</div>
                <!-- 如果不需要按钮，下面可以删掉 -->
                <!-- <button class="btn btn--primary" @click="boxDisabled = false">我知道了</button> -->
            </div>
        </div>
        ...
        <div class="table-wrapper">
            ...
            <div v-else class="table-scroll">
                <vxe-table v-if="tableObjects.length || !loadingList" ref="xTableRef" :data="tableObjects" border round
                    ...
                    <!-- 动态列：按 headerKeys 生成 -->
                    <vxe-column v-for="key in headerKeys" :key="key" :field="key" :title="key" show-overflow />
...
const innerPath = ref('oss://ym-west-data/data/tmp/optimization_new.csv')
...
.csv-box-mask {
    ...
    align-items: center;
    justify-content: center;
    pointer-events: auto; // 自身可接收事件，拦截下面的点击
